export function remove(imageBuffer, options) {
...
export function removeMultiple(imageBuffers, options) {
  if (options == undefined) {
    options = {};
  }

  return imageBuffers.map((imageBuffer) =>
    remove(imageBuffer, options.verbose),
  );
}
